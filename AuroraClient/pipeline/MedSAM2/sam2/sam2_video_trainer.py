...
class SAM2VideoTrainer(nn.Module):
    ...
    def __init__(self, model_cfg, sam2_checkpoint, device, memory_size=7, mask_threshold=0.5, use_mask_threshold=False):
        ...
        # Spatial dim for backbone feature maps (stride 4/8/16). Must follow
        # image_size: 1024 → 256/128/64, 512 → 128/64/32. A 512 clip with the
        # 1024 sizes still has a matching numel, so .view() raises a stride error.
        image_size = int(getattr(self.model, "image_size", 1024) or 1024)
        self._bb_feat_sizes = [
    ...
    def forward(self, videos, bboxes, labels=None, dense_prompt_mask=None):
        """
        ...
        first_frame_masks, first_frame_logits, first_frame_ious, object_score_logits = (
            self._predict_first_frame(
                first_frame_features, first_frame_bbox, dense_prompt_mask=dense_prompt_mask
            )
        )
    ...
    def _embed_bbox(self, bbox, dense_mask=None):
        """
        Embed bounding boxes, optionally with a dense mask prompt.
        """
        ...
        if dense_mask is not None:
            ...
            target = self.model.sam_prompt_encoder.mask_input_size
            mask_in = F.interpolate(
                dense_mask.float(), size=target, mode="bilinear", align_corners=False
            )
        sparse_embeddings, dense_embeddings = self.model.sam_prompt_encoder(
            points=concat_points, boxes=None, masks=mask_in
        )
        return sparse_embeddings, dense_embeddings

    def _predict_first_frame(self, features, bbox, dense_prompt_mask=None):
        """
        Predict masks and IoUs for the first frame.
        """
        sparse_embeddings, dense_embeddings = self._embed_bbox(
            bbox, dense_mask=dense_prompt_mask
        )
    ...
    def _predict_frame(self, features, memory, prev_mask=None):
        ...
        pix_feat_with_mem = pix_feat_with_mem.permute(1, 2, 0).contiguous().view(
